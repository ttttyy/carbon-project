...
func CreateSelling(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	if val, err := strconv.ParseFloat(size, 64); err != nil {
		return shim.Error(fmt.Sprintf("size参数格式转换出错: %s", err))
	} else {
		formattedSize = val
	}
	...
	// 写入账本
	if err := utils.WriteLedger(selling, stub, lib.SellingKey, []string{selling.Seller, size, price}); err != nil {
...
func CreateSellingByBuy(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	resultsSelling, err := utils.GetStateByPartialCompositeKeys2(stub, lib.SellingKey, []string{seller, size})
	...
	//}
	//判断余额是否充足
	if buyerAccount.Balance < selling.Price*selling.Size {
		return shim.Error(fmt.Sprintf("碳售价为%f,您的当前余额为%f,购买失败", selling.Price, buyerAccount.Balance))
	}
	...
	buyerAccount.Balance -= selling.Price * selling.Size
...
// 1、当前处于saleStart销售状态
// 2、当前处于delivery交付中状态
func closeSelling(closeStart string, selling lib.Selling, sellingBuy lib.SellingBuy, buyer string, stub shim.ChaincodeStubInterface) ([]byte, error) {
